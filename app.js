(function () {
  ...
  // Данные из графика на апрель 2026
  const scheduleData = [
  ...
  function renderTable() {
    ...
    scheduleData.forEach(row => {
      ...
      for (let i = 1; i <= 30; i++) {
        const v = row.shifts[i] || "";
        let c = v === "1219" ? "cell-1219" : v === "1202" ? "cell-1202" : v === "ОТП" ? "cell-otp" : "";
  ...
  fetch("data/instructions.json").then(r => r.json()).then(json => {
    ...
    json.categories.forEach(c => {
      ...
      b.type = "button"; b.className = "action-main-btn"; b.textContent = c.title;
      b.onclick = () => {
